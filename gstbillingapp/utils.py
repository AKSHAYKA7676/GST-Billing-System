...
def invoice_data_processor(invoice_post_data):
    ...
    # Basic invoice info
    processed['invoice_number'] = invoice_post_data['invoice-number']
    processed['invoice_date'] = invoice_post_data['invoice-date']

    processed['customer_name'] = invoice_post_data['customer-name']
    processed['customer_address'] = invoice_post_data['customer-address']
    processed['customer_phone'] = invoice_post_data['customer-phone']
    processed['customer_gst'] = invoice_post_data['customer-gst']

    processed['vehicle_number'] = invoice_post_data.get('vehicle-number', '')

    processed['igstcheck'] = ('igstcheck' in invoice_post_data)

    processed['items'] = []

    # OLD totals (kept for compatibility)
    processed['invoice_total_amt_without_gst'] = float(invoice_post_data['invoice-total-amt-without-gst'])
    processed['invoice_total_amt_sgst'] = float(invoice_post_data['invoice-total-amt-sgst'])
    processed['invoice_total_amt_cgst'] = float(invoice_post_data['invoice-total-amt-cgst'])
    processed['invoice_total_amt_igst'] = float(invoice_post_data['invoice-total-amt-igst'])
    processed['invoice_total_amt_with_gst'] = float(invoice_post_data['invoice-total-amt-with-gst'])
    ...
    for idx, product in enumerate(invoice_post_data['invoice-product']):
        ...
        item['product_hsn'] = invoice_post_data['invoice-hsn'][idx]
        item['unit'] = invoice_post_data['invoice-unit'][idx]
        item['qty'] = float(invoice_post_data['invoice-qty'][idx])

        item['gst_percent'] = float(invoice_post_data['invoice-gst-percentage'][idx])

        # OLD values (kept)
        item['invoice_rate_with_gst'] = float(invoice_post_data['invoice-rate-with-gst'][idx])
        item['invoice_amt_with_gst'] = float(invoice_post_data['invoice-amt-with-gst'][idx])

        item['invoice_rate_without_gst'] = float(invoice_post_data['invoice-rate-without-gst'][idx])
        item['invoice_amt_without_gst'] = float(invoice_post_data['invoice-amt-without-gst'][idx])

        item['invoice_amt_cgst'] = float(invoice_post_data['invoice-amt-cgst'][idx])
        item['invoice_amt_sgst'] = float(invoice_post_data['invoice-amt-sgst'][idx])
        item['invoice_amt_igst'] = float(invoice_post_data['invoice-amt-igst'][idx])

        # ======================================================
        #               NEW GST SYSTEM 2.0 LOGIC
        # ======================================================

        # Value without GST
        item['value_without_gst'] = item['invoice_amt_without_gst']

        # Split GST 50-50 (CGST/SGST)
        item['cgst_value'] = round(item['invoice_amt_cgst'], 2)
        item['sgst_value'] = round(item['invoice_amt_sgst'], 2)

        # Total
        item['total_value_with_gst'] = item['value_without_gst'] + item['cgst_value'] + item['sgst_value']

        # Add to list
        processed['items'].append(item)

        # Update totals
        total_without_gst += item['value_without_gst']
        total_cgst += item['cgst_value']
        total_sgst += item['sgst_value']
        total_with_gst += item['total_value_with_gst']

    # Assign GST 2.0 totals
    processed['total_without_gst'] = round(total_without_gst, 2)
    processed['total_cgst'] = round(total_cgst, 2)
    processed['total_sgst'] = round(total_sgst, 2)
    processed['invoice_total_amt_with_gst'] = round(total_with_gst, 2)
